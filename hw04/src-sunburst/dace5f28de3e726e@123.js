...
<h3>從上圖中，我們可以看出：
  <ul>
    <li>工作室以及替代空間的的藝術工作者占此問卷的超過半數</li>
    <li>減少包裝及再生能源為減少碳排放量的趨勢</li>
    <li>工作室的藝術工作者由於人數眾多所以有很多種不同減少碳排放量的方式</li>
  </ul>
</h3>`
)}
...
function _confirm(Inputs){return(
Inputs.button("是", {label: "是否顯示結論"})
)}

function _23(confirm,md){return(
confirm ? md`工作室以及替代空間的的藝術工作者占此問卷的超過半數` : md`等待中`
)}
...
md`正相關性： x軸和y軸呈現正相關，表示回答者對於這兩個問題的了解程度有相似的趨勢。這可能意味著對於2050淨零排放的了解程度和另一問題的了解程度存在某種相關性。

2050淨零排放的了解程度普遍較低： 大部分的回答者對於「2050淨零排放」政策的了解程度大約為2，代表對於這個政策的內容相對較少。這可能顯示該政策需要更多的宣傳和教育，以提高公眾對其內容的了解。

極少數人有較高了解程度： 換句話說，有一小部分的受訪者對於「2050淨零排放」政策有較高的了解程度。這些人可能是政策相關領域的專業人士，環保愛好者或者其他特定群體。`
)}
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("viewof confirm")).define("viewof confirm", ["Inputs"], _confirm);
  main.variable(observer("confirm")).define("confirm", ["Generators", "viewof confirm"], (G, _) => G.input(_));
  main.variable(observer()).define(["confirm","md"], _23);
